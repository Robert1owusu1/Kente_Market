...
function IconButton({ children, text, onClick, disabled, loading, variant = "outline", ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { text: string; loading?: boolean; variant?: "outline" | "solid" }) {
...
function PasswordStrengthIndicator({ password }: { password?: string }) {
  const getPasswordStrength = (password?: string): {
    ...
    color: string;
    width: string;
    checks?: Array<{ test: RegExp; label: string; passed: boolean }>;
  } => {
...
function IconInput({ 
  ...
  ...props 
}: React.InputHTMLAttributes<HTMLInputElement> & { label?: string; error?: string; showPasswordStrength?: boolean; children: React.ReactNode }) {
  ...
  return (
    <div className="w-full">
      ...
      {showPasswordStrength && type === "password" && (
        <PasswordStrengthIndicator password={value as string} />
      )}
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const registerError = err as { data?: { message?: string }; status?: number; name?: string; message?: string };
    ...
    if (registerError.data?.message) {
      ...
      switch (registerError.status) {
        ...
        case 500:
          ...
          break;
        default:
          errorMessage = `Registration failed (Error ${registerError.status})`;
      }
    } else if (registerError.name === 'NetworkError' || registerError.message?.includes('fetch')) {
