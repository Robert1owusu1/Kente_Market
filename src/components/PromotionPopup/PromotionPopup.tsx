...
type PopupPromotion = Promotion & {
  showAsPopup?: boolean;
  popupDismissedExpiryHours?: number;
  bgColor?: string;
  textColor?: string;
  linkText?: string;
};
...
const PromotionPopup = () => {
  ...
  const [active, setActive] = useState<PopupPromotion | null>(null);
  ...
  useEffect(() => {
    ...
    const eligible = popups.find((p) => {
      const popup = p as PopupPromotion;
      if (!popup.showAsPopup) return false;
      const d = dismissed[(p.id as number | string) as string];
      if (!d) return true;
      const hours = popup.popupDismissedExpiryHours || 24;
      return now - d > hours * 3600 * 1000;
    });

    if (eligible) {
      setActive(eligible as PopupPromotion);
  ...
  const dismiss = () => {
    ...
    setTimeout(() => {
      if (active) {
        const dismissed = getDismissed();
        dismissed[(active.id as number | string) as string] = Date.now();
