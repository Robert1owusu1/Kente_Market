...
const DECOR_TEMPLATES: DecorTemplate[] = [
  {
    id: "table-runner",
    name: "Table Runner",
    icon: <FaHome />,
...
const DecorateMode = ({ product, onProductChange }: DecorateModeProps) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [roomImage, setRoomImage] = useState<string | null>(null);
  const [template, setTemplate] = useState<DecorTemplate>(DECOR_TEMPLATES[0]);
  ...
  const handleTemplateChange = (t: DecorTemplate) => {
    setTemplate(t);
  };
