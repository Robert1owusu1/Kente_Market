...
interface VendorReviewRow extends Review {
  productTitle?: string;
  name?: string;
...
const VendorReviews = () => {
  const { data: reviews = [], isLoading } = useGetVendorReviewsQuery() as {
    data?: VendorReviewRow[];
    isLoading: boolean;
  };
  const [searchTerm, setSearchTerm] = useState('');

  const filteredReviews = reviews.filter((r: VendorReviewRow) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg p-5 shadow-md text-center">
          ...
          <div className="flex justify-center gap-1 mt-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <FaStar key={s} className={`text-sm ${s <= Math.round(Number(avgRating)) ? 'text-amber-400' : 'text-gray-300'}`} />
            ))}
          </div>
      ...
      {filteredReviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredReviews.map((review: VendorReviewRow) => (
            <div key={review.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
              ...
              <p className="text-xs text-gray-400 mt-2">{new Date(review.created_at as string).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
      )}
