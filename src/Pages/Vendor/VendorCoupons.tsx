...
const emptyForm: CouponForm = { code: '', discountType: 'percentage', discountValue: '', minPurchase: '', maxUses: '', expiresAt: '', isActive: true };
...
const VendorCoupons = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm((prev) => ({ ...prev, [name]: type === 'checkbox' ? checked : value }) as CouponForm);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.discountValue || isNaN(Number(form.discountValue)) || parseFloat(form.discountValue) <= 0) {
      return toast.error('Discount value must be greater than 0');
    }
    try {
      if (editing) {
        await updateCoupon({ id: editing.id as number | string, ...form }).unwrap();
        toast.success('Coupon updated');
      } else {
        await createCoupon(form as unknown as Record<string, unknown>).unwrap();
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => handleDelete(deleteConfirm.id as number | string)} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Delete</button>
            </div>
          </div>
        </div>
      )}
