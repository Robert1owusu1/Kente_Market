...
const StockRow = ({ item }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateProduct({ id: item.id, stock, lowStockThreshold: isNaN(lowStockThreshold) ? 0 : lowStockThreshold }).unwrap();
...
const VendorInventory = () => {
  const { data, isLoading, isError } = useGetVendorInventoryQuery();
    ...
    <div className="space-y-6">
      ...
      {(summary.lowStock > 0 || summary.outOfStock > 0) && (
        <div className="flex items-center gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-800 rounded-lg p-4 text-amber-800 dark:text-amber-300 text-sm">
          ...
          {summary.outOfStock > 0 && <span>{summary.outOfStock} product(s) are out of stock.</span>}
          {summary.lowStock > 0 && <span>{summary.lowStock} product(s) are running low on stock.</span>}
        </div>
      )}
