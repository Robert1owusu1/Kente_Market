  ...
  const handleProgress = async (order) => {
    ...
    try {
      await updateStatus({
        ...
        productionNote: noteText[order.id] || undefined,
        expectedCompletionDate: completionDate[order.id] || undefined,
      }).unwrap();
      toast.success('Order updated — customer has been notified');
      setNoteText((s) => ({ ...s, [order.id]: '' }));
      setCompletionDate((s) => ({ ...s, [order.id]: '' }));
      refetch();
    } catch (err) {
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredOrders.map((order) => {
            const isCustomised = (Array.isArray(order.items) ? order.items : []).some((it) => it.isCustomizable);
                  ...
                  <div className="mb-4 bg-gray-50 dark:bg-gray-700/40 rounded-lg p-3">
                    <div className="flex flex-col sm:flex-row gap-3">
                      <div className="flex-1">
                        ...
                        <input
                          type="date"
                          value={completionDate[order.id] || ''}
                          onChange={(e) => setCompletionDate((s) => ({ ...s, [order.id]: e.target.value }))}
                      ...
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          onChange={(e) => setNoteText((s) => ({ ...s, [order.id]: e.target.value }))}
