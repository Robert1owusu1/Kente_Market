...
const VendorPayouts = () => {
  const { data, isLoading } = useGetMyVendorProfileQuery();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
          ...
          <p className="text-2xl font-bold mt-1 text-green-600">{formatCurrency(summary.availableBalance || 0)}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
          <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-2">
            <FaCheckCircle className="text-green-500" /> Total Earned
          </p>
          <p className="text-2xl font-bold mt-1">{formatCurrency(summary.totalEarned || 0)}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
          <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-2">
            <FaExclamationTriangle className="text-red-400" /> Held in Escrow
          </p>
          <p className="text-2xl font-bold mt-1">{formatCurrency(summary.pendingPayout || 0)}</p>
        </div>
      </div>
              ...
              <tbody>
                {payouts.slice(0, 20).map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    ...
                    <td className="py-3 pr-4">{formatCurrency(p.amount)}</td>
                    <td className="py-3 pr-4 text-gray-500">- {formatCurrency(p.platformFee)}</td>
                    <td className="py-3 pr-4 font-semibold">{formatCurrency(p.payoutAmount)}</td>
                    <td className="py-3">
