...
import type { VendorStaff } from '../../slices/apiTypes';

interface StaffRow extends VendorStaff {
  status?: string;
  permissions?: string[];
}
...
const VendorStaff = () => {
  ...
  const { data: staff = [], isLoading, isError } = useGetVendorStaffQuery() as {
    data?: StaffRow[];
    isLoading: boolean;
    isError: boolean;
  };
  ...
  const handleDelete = async (id: number | string | undefined, name: string | undefined) => {
    if (!window.confirm(`Remove ${name}? They will lose access immediately.`)) return;
    try {
      await deleteStaff(id as number | string).unwrap();
