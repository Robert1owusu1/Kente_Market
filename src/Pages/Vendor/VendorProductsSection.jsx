...
import { FaTimes, FaImage, FaPlus, FaTrash, FaSpinner, FaCheckCircle, FaClock, FaEdit, FaSearch, FaFilter } from 'react-icons/fa';
...
const VendorProductsSection = ({ vendorStatus }) => {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      const msg = error?.data?.message || 'Failed to upload image';
      toast.error(msg);
      setImage('');
    }
  };
              ...
              <tbody>
                {filteredProducts.map((p) => (
                  <tr key={p.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 font-semibold text-primary">GH₵{parseFloat(p.price || 0).toFixed(2)}</td>
