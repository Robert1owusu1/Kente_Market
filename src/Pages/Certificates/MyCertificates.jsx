    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
      ...
      {isError && (
        <p className="mt-4 text-sm text-red-600 dark:text-red-400">
          {error?.data?.message || 'No certificate found for that number.'}
        </p>
      )}
      {data && !isError && (
        <div className={`mt-4 rounded-lg p-4 ${data.verified ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
          {data.certificate ? (
            <>
              <p className="font-semibold text-gray-900 dark:text-white mb-1">
                {data.verified ? 'Authentic — verified' : 'Verification failed'}
              </p>
              <p className="text-sm text-gray-700 dark:text-gray-300">
                {data.verified
                  ? `${data.certificate.certificateNumber} — ${data.certificate.patternName}, woven by ${data.certificate.weaverName || 'a verified artisan'}, registered ${formatDate(data.certificate.dateRegistered)}.`
                  : data.message}
              </p>
            </>
          ) : (
