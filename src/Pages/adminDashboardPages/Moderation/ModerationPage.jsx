...
const ModerationPage = () => {
  ...
  const { data: products = [], isLoading, isError, refetch } = useGetModerationProductsQuery(statusFilter);
  ...
  const handleModerate = async (product, status) => {
    ...
    try {
      ...
      toast.error(err?.data?.message || 'Failed to moderate product');
    }
  };
          ...
          <div key={p.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400 mt-1">Listed {formatDate(p.created_at)}</p>
              </div>
