...
const CouponsPage = () => {
  const { data: rawCoupons = [], isLoading } = useGetCouponsQuery();
  const coupons = rawCoupons as CouponRow[];
  ...
  const openEdit = (coupon: CouponRow) => {
    ...
    setForm({
      ...
      minPurchase: (coupon.minPurchase || '') as string,
      maxUses: (coupon.maxUses || '') as string,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!form.discountValue || isNaN(form.discountValue as unknown as number)) {
    ...
    try {
      ...
      toast.error(errorMessage(err) || 'Failed to save coupon');
    }
  };
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      ...
      toast.error(errorMessage(err) || 'Failed to delete coupon');
    }
  };
              ...
              <tbody>
                {coupons.map((coupon) => (
                  <tr key={coupon.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">
                      {formatDate(coupon.expiresAt as string)}
                    </td>
