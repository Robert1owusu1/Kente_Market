...
const ReviewsAdminPage = () => {
  ...
  const adminReviews = reviews as unknown as AdminReview[];
  const adminReports = reports as unknown as AdminReport[];
  ...
  const reportedReviews = adminReports.filter((r) => r.status === 'pending');
  ...
  const handleReportAction = async (reportId: number | string, status: string) => {
    try {
                ...
                <div key={report.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">
                      Review #{String(report.reviewId)} — {report.reason || 'No reason'}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Reported by user #{String(report.userId || 'N/A')} • {formatDate(report.createdAt || report.created_at || '')}
                    </p>
                  </div>
