...
const ProductsPage = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (query) {
      filtered = filtered.filter(product => {
        ...
        const tag = ((product.tag as string) || '').toLowerCase();
  ...
  const handleSubmitProduct = async (productData: Record<string, unknown>) => {
    try {
      if (selectedProduct) {
  ...
  if (error) {
    ...
    return <ErrorDisplay message={err?.data?.message || 'Failed to load products'} />;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-md">
          ...
          <p className="text-2xl font-bold text-blue-600">{formatCurrency(stats.avgPrice, undefined)}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-md">
              ...
              <tbody>
                {filteredProducts.map((product) => (
                  <tr key={product.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td>
                      <div>
                        <p className="font-semibold text-gray-800 dark:text-white">{formatCurrency(product.price, undefined)}</p>
                        {product.originalPrice && Number(product.originalPrice) > Number(product.price) && (
                          <p className="text-xs text-gray-500 line-through">{formatCurrency(product.originalPrice, undefined)}</p>
                        )}
                      </div>
                    </td>
                    <td>
