...
const ImageUpload = ({ currentImage, onImageChange, isLoading }: ImageUploadProps) => {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      onImageChange(response.image as string);
      toast.success('Image uploaded successfully');
    } catch (error) {
      const err = error as { data?: { message?: string }; message?: string; error?: string; status?: string } | undefined;
      console.error('Upload error:', error);
      // Surface the most specific message available:
      // - HTTP errors expose error.data.message (e.g. 400/401/413/500)
      // - timeouts/network failures expose error.error (e.g. TIMEOUT_ERROR)
      const message =
        err?.data?.message ||
        (err?.error ? `Upload failed: ${err.error}` : null) ||
        (err?.status === 'TIMEOUT_ERROR'
          ? 'Upload timed out. Please try a smaller image or try again.'
...
const ProductFormModal = ({ product, onClose, onSubmit, isLoading }: ProductFormModalProps) => {
  const { data: vendors = [] } = useGetAllVendorsQuery(undefined);
  ...
  const [formData, setFormData] = useState<ProductFormState>({
    ...
    fabricType: (product?.fabricType || '') as string,
    productionTime: (product?.productionTime || '') as string,
    tag: (product?.tag || '') as string,
    isCustomizable: product?.isCustomizable || false,
    featured: product?.featured || false,
    basePrice: (product?.basePrice || '') as string,
    vendorId: (product?.vendorId || '') as string,
  });

  // Product categories come from the admin-manageable categories list
                ...
                >
                  ...
                  {approvedVendors.map(v => (
                    <option key={v.id} value={v.id}>
                      {v.businessName} — {v.firstName as string} {v.lastName as string}
                    </option>
                  ))}
                </select>
