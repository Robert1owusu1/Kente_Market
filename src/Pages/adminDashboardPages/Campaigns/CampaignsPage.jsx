        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min="1" value={discountValue} onChange={(e) => setDiscountValue(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
...
const CampaignsPage = () => {
  ...
  const { data: campaigns = [], isLoading, isError } = useGetCampaignsQuery(showAll);
          ...
          {campaigns.map((c) => (
            <div key={c.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5">
              ...
              <div className="mt-3 text-sm text-gray-600 dark:text-gray-400 space-y-1">
                <p className="flex items-center gap-1"><FaCalendarAlt /> <span className="line-through">{formatDate(c.startsAt)}</span></p>
                <p className="flex items-center gap-1"><FaCalendarAlt /> Ends {formatDate(c.endsAt)}</p>
              </div>
