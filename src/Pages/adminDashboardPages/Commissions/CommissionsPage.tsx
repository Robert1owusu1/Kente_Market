...
import type { CommissionRule } from '../../../slices/apiTypes';

interface CommissionRuleRow extends CommissionRule {
  id: number | string;
  scope: string;
...
const CreateRuleModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error(errorMessage(err) || 'Failed to create rule');
    }
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="number" step="0.01" min="0" max="0.5" value={rate} onChange={(e) => setRate(e.target.value as unknown as number)}
              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-white" />
          </div>
...
const CommissionsPage = () => {
  ...
  const { data: rawRules = [], isLoading, isError } = useGetCommissionRulesQuery();
  const rules = rawRules as CommissionRuleRow[];
              ...
              <tbody>
                {rules.map((r) => (
                  <tr key={r.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="text-sm font-semibold text-gray-800 dark:text-white">{((r.rate as number) * 100).toFixed(1)}%</td>
                    <td className="text-sm text-gray-500 dark:text-gray-400">
                      {new Date(r.created_at as string).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                    </td>
                    <td>
                      <button
