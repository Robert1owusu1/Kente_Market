...
import type { Certificate } from '../../../slices/apiTypes';
...
const IssueModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error(errorMessage(err) || 'Failed to issue certificate');
    }
  };
...
const CertificatesAdminPage = () => {
  ...
  const { data: rawCertificates = [], isLoading, isError } = useGetAllCertificatesQuery();
  const certificates = rawCertificates as CertificateRow[];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {certificates.map((c) => (
            <div key={c.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 border-l-4 border-amber-500">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Registered {formatDate(c.dateRegistered as string)}</p>
            </div>
          ))}
        </div>
      )}
