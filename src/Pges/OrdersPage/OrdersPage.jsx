// Pges/OrdersPage/OrdersPage.jsx
...
const escrowLabel = (s) =>
  ({ held: 'Escrow', released: 'Released', releasing: 'Releasing', failed: 'Failed', none: '' }[s] || '');
  ...
  if (isError) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <p className="text-red-500 mb-4">{error?.data?.message || error?.message || 'Failed to load orders'}</p>
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {order.escrowStatus && order.escrowStatus !== 'none' && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400">
                      <FaShieldAlt /> {escrowLabel(order.escrowStatus)}
                    </span>
                  )}
                </div>
              </div>
